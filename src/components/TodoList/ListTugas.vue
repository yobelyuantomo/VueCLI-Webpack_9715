<template>
    <v-main class="list">
        ...
        <div v-if="selectedCheckbox.length > 0">
            <v-card class="mt-3 pb-3">
                <v-card-title>
                    Delete Multiple
                </v-card-title>
                <ul class="ml-4">
                    <li v-for="item in selectedCheckbox" :key="item.task">
                        {{ item.task }}
                    </li>
                </ul>
                <v-btn color="red ml-3 mt-4 mb-3" dark @click="multipleDelete()">
                    Hapus Semua
                </v-btn>
            </v-card>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            multipleDelete(){
                this.selectedCheckbox.forEach(x => this.todosFinished.push(this.todos[this.todos.indexOf(x)]));
                this.selectedCheckbox.forEach(x => this.todos.splice(this.todos.indexOf(x),1));
